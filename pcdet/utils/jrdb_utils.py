...
_T_upper_lidar_to_base = np.array([0, 0, 0.33529], dtype=np.float32).reshape(3, 1) # from Dan 
# _T_upper_lidar_to_base = np.array([0, 0, -0.33529], dtype=np.float32).reshape(3, 1) # from calib file, actually to rgb 
# _T_upper_lidar_to_base = np.array([0.019685, 0, -1.077382], dtype=np.float32).reshape(3, 1) # from calib file, actually to rgb 
_R_upper_lidar_to_base = _get_R_z(_rot_z_upper_lidar_to_base)

# lower_lidar to base
_rot_z_lower_lidar_to_base = 0.0
_T_lower_lidar_to_base = np.array([0, 0, -0.13511], dtype=np.float32).reshape(3, 1) # from Dan 
# _T_lower_lidar_to_base = np.array([0, 0, 0.13511], dtype=np.float32).reshape(3, 1) # from calib file, actually to rgb 
# _T_lower_lidar_to_base = np.array([0.019685, 0, -0.606982], dtype=np.float32).reshape(3, 1) # from report 
_R_lower_lidar_to_base = np.eye(3, dtype=np.float32)

_T_lower_to_upper_lidar = np.array([0,0, 1.077382 - 0.606982], dtype=np.float32).reshape(3, 1) 
_R_lower_to_upper_lidar = _get_R_z(_rot_z_upper_lidar_to_base)

_T_camera_to_base = np.array([0.019685, 0, -0.742092], dtype=np.float32).reshape(3, 1)
...
def transform_pts_lower_to_upper_velodyne(pts):
    return _R_lower_to_upper_lidar @ pts + _T_lower_to_upper_lidar
