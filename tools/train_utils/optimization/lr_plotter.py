...
from fastai_optim import OptimWrapper
# from . import OptimWrapper
# 
# from tools.train_utils.fastai_otim import OptimWrapper
...
#     # total_step_size = 108720
#     # epochs = [30, 50, 80]
#     num_epochs = 30 
#     total_step_size = num_epochs * 3624
#     schd = OneCycle(opt, total_step_size, 0.01, (0.95, 0.85), 100.0, 0.4)


#     lrs = []
#     moms = []
#     for i in range(total_step_size):
#         schd.step(i)
#         lrs.append(opt.lr)
#         moms.append(opt.mom)
...
#     plt.plot(lrs, label="30")
#     plt.legend(loc='upper right', title='# epochs', frameon=True, fontsize='medium')
#     plt.vlines(total_step_size/2, 0.0, 0.01, color='gray', alpha=0.3, linestyles='dashed')
#     plt.savefig(plot_dir+'lrs_1.png', bbox_inches='tight')  
#     # plt.show()
#     plt.plot(moms)
#     plt.savefig(plot_dir+'moms_1.png', bbox_inches='tight')  
#     # plt.show()
    COLORS = [
    ...
    wd = [0.3, 0.4]#[0.3, 0.4, 0.5]
    div_factor = [10, 100]
    max_lr = 0.01

    color_ct = 0 
    for pct_start in wd: 
        for div in div_factor: 
            start_lr = max_lr/div
            color_ct += 1
            schd = OneCycle(opt, total_step_size, max_lr, (0.95, 0.85), div, pct_start)
            lrs = []
            moms = []
            for i in range(total_step_size):
                schd.step(i)
            ...
            plt.xticks(fontsize=14)
            plt.yticks(fontsize=14)
            plt.plot(lrs, label="rise: {}\nstart_lr: {}".format(pct_start, start_lr), color=COLORS[color_ct])
            plt.legend(loc='upper right', title='Parameters', frameon=True, fontsize='medium')
            plt.vlines(total_step_size/2, 0.0, 0.01, color='gray', alpha=0.3, linestyles='dashed')
    plt.savefig(plot_dir+'lrs_2.pdf', bbox_inches='tight')  
    # plt.show()
    plt.plot(moms)
    plt.savefig(plot_dir+'moms_1.png', bbox_inches='tight')  